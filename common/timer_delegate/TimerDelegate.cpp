    ...
    {
        {
            ...
            cv_.wait(lk, [&] { return stop_flag_ || current_strategy_ != nullptr; });
            ...
            if (timeout < 0)
            {
                cv_.wait(lk, [&] { return stop_flag_ || strategy_changed_ || reset_flag_; });
                continue;
            }

            const bool expired = !cv_.wait_for(lk, std::chrono::milliseconds(timeout),
